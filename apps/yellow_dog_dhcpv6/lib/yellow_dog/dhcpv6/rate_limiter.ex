defmodule YellowDog.Dhcpv6.RateLimiter do
  ...
  ## Parameters
  - `client_id` - Client identifier (typically DUID or IPv6 address)
  ...
  def init(opts) do
    ...
    :ets.new(:dhcpv6_rate_buckets, [:named_table, :public, read_concurrency: true])
    ...
    :telemetry.execute(
      [:yellow_dog, :dhcpv6, :rate_limiter, :started],
      %{count: 1},
  ...
  def handle_call({:check_rate, client_id}, _from, state) do
    ...
    if not global_allowed do
      :telemetry.execute(
        [:yellow_dog, :dhcpv6, :rate_limiter, :denied],
        %{count: 1},
  ...
  def handle_call(:stats, _from, state) do
    bucket_count = :ets.info(:dhcpv6_rate_buckets, :size)
  ...
  def handle_call(:reset, _from, state) do
    :ets.delete_all_objects(:dhcpv6_rate_buckets)
  ...
  defp load_config(opts) do
    app_config = Application.get_env(:yellow_dog_dhcpv6, __MODULE__, [])
  ...
  defp check_client_limit(client_key, config, now) do
    case :ets.lookup(:dhcpv6_rate_buckets, client_key) do
      [] ->
        # New client, create bucket with one token consumed
        initial_tokens = config.client_tokens - 1
        :ets.insert(:dhcpv6_rate_buckets, {client_key, initial_tokens, now})
        {true, initial_tokens}
      ...
      [{^client_key, tokens, last_update}] ->
        ...
        if current_tokens >= 1 do
          ...
          :ets.insert(:dhcpv6_rate_buckets, {client_key, new_tokens, now})
          {true, new_tokens}
        else
          # Update last_update for accurate refill calculation
          :ets.insert(:dhcpv6_rate_buckets, {client_key, current_tokens, now})
  ...
  defp cleanup_expired_buckets(ttl) do
    ...
    Enum.each(expired, &:ets.delete(:dhcpv6_rate_buckets, &1))

    if length(expired) > 0 do
      :telemetry.execute(
        [:yellow_dog, :dhcpv6, :rate_limiter, :cleanup],
