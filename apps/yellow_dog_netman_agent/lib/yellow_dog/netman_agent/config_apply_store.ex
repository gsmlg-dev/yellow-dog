defmodule YellowDog.NetmanAgent.ConfigApplyStore do
  @moduledoc """
  ...
  @attempt_keys ~w(
    ...
    previous rollback status version
  )
  ...
  @candidate_keys ~w(
    digest expected_revision operation payload published_at
  ...
  @doc "Reads durable boot evidence without starting the apply store."
  @spec read_boot_state(Path.t(), term()) ::
          {:ok, %{known_good: map() | nil, runtime_status: atom()}}
          | {:error, :missing | :corrupt | :invalid_options}
  def read_boot_state(data_dir, netman_id) do
    with {:ok, data_dir} <- absolute_data_dir(data_dir),
         {:ok, netman_id} <- netman_id(netman_id),
         config = %{data_dir: data_dir, netman_id: netman_id, storage_opts: []},
  ...
  defp do_transition(:provisional, %{version: version}, snapshot, config) do
    cond do
      match_attempt?(snapshot, version, :applying, :provisional) and
      ...
      match_attempt?(snapshot, version, :applying, :before_activate) ->
        update_attempt_and_runtime(snapshot, config, :unknown, fn attempt ->
          %{attempt | checkpoint: :provisional}
        end)
  ...
  defp do_transition(:applied, %{version: version}, snapshot, config) do
    cond do
      ...
      Enum.any?([:before_activate, :provisional], fn checkpoint ->
  ...
  defp same_candidate_preflight(
         %{attempt: %{status: :applying, checkpoint: :provisional}} = snapshot
       ),
       do: {:replay, snapshot}
  ...
  defp validate_candidate(candidate, config) when is_map(candidate) do
    with true <- exact_keys?(candidate, @candidate_keys),
         ...
         "netman" <- candidate["target_type"],
         true <- candidate["target_id"] == config.netman_id,
  ...
  defp same_candidate?(candidate, attempt) do
    candidate.target_id == attempt_target_id(attempt, candidate.target_id) and
      candidate.version == attempt.version and
  ...
  defp applying_checkpoint?(snapshot, version) do
    Enum.any?([:before_install, :before_activate, :provisional, :unknown], fn checkpoint ->
      match_attempt?(snapshot, version, :applying, checkpoint)
    end)
  end
  ...
  defp decode_snapshot(document, config) when is_map(document) do
    with true <- exact_keys?(document, @top_keys),
         ...
         {:ok, outbox} <- decode_outbox(document["outbox"], config.netman_id),
  ...
  defp decode_attempt(value, _config) when is_map(value) do
  ...
  defp decode_outbox(value, netman_id) when is_list(value) and length(value) <= 3 do
    Enum.reduce_while(value, {:ok, []}, fn entry, {:ok, entries} ->
      case decode_outbox_entry(entry, netman_id) do
  ...
  defp decode_outbox(_value, _netman_id), do: {:error, :corrupt}

  defp decode_outbox_entry(value, netman_id) when is_map(value) do
    with true <- exact_keys?(value, @outbox_keys),
         ...
         {:ok, %ConfigState{target_type: :netman, target_id: ^netman_id} = message} <-
  ...
  defp decode_outbox_entry(_value, _netman_id), do: {:error, :corrupt}
  ...
  defp coherent_attempt_state(
       ...
       when not is_nil(previous) and is_binary(revision),
       do: :ok
  ...
  defp coherent_attempt_state(
         ...
         %{runtime_status: :unconfigured, known_good: nil}
       ),
       do: :ok
  ...
  defp coherent_outbox(%{attempt: attempt, outbox: outbox, target_id: netman_id}) do
    ...
    with true <- suffix?(expected_states, actual_states),
         true <- Enum.all?(outbox, &coherent_message?(&1.message, attempt, netman_id)) do
      :ok
    else
      _invalid -> {:error, :corrupt}
    end
  ...
  defp coherent_message?(message, attempt, netman_id) do
    base =
      message.target_type == :netman and message.target_id == netman_id and
  ...
  defp config_operation(name, payload) do
    with true <- name in @config_operations,
         {:ok, %Operation{target_type: :netman, kind: :config} = operation} <-
  ...
  defp config_operation_name(name) do
    with true <- name in @config_operations,
         {:ok, %Operation{target_type: :netman, kind: :config}} <- Operation.lookup(name) do
      {:ok, name}
    else
      _invalid -> {:error, :corrupt}
    end
  ...
  defp ensure_owned_path(config) do
    [config.data_dir, netman_directory(config)]
  ...
  defp netman_directory(config), do: Path.join(config.data_dir, "netman")
  defp apply_state_path(config), do: Path.join(netman_directory(config), "apply_state.json")
