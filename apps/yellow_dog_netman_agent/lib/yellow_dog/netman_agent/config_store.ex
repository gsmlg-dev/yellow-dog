defmodule YellowDog.NetmanAgent.ConfigStore do
  @moduledoc """
  Durable immutable staging for local Netman configuration deliveries.
  """
  ...
  @restore_checkpoint_keys Enum.sort([
                             "candidate_digest",
                             "candidate_operation",
                             "restore_operation",
                             "restore_payload",
  ...
  @enforce_keys [:data_dir, :netman_id, :max_versions, :storage_opts]
  ...
  @spec fetch(pos_integer(), String.t(), GenServer.server()) ::
          {:ok, map()} | {:error, Error.t()}
  def fetch(version, digest, server \\ __MODULE__),
    do: GenServer.call(server, {:fetch, version, digest})

  @doc "Fetches a durable staged document by its installed payload revision."
  @spec fetch_revision(String.t(), GenServer.server()) ::
          {:ok, map()} | {:error, Error.t()}
  def fetch_revision(revision, server \\ __MODULE__),
    do: GenServer.call(server, {:fetch_revision, revision})

  @doc "Durably records the exact operation-specific state needed to undo a candidate."
  @spec put_restore_checkpoint(pos_integer(), String.t(), map(), GenServer.server()) ::
          :ok | {:error, Error.t()}
  def put_restore_checkpoint(version, digest, checkpoint, server \\ __MODULE__),
    do: GenServer.call(server, {:put_restore_checkpoint, version, digest, checkpoint})

  @doc "Fetches a candidate's durable operation-specific restore checkpoint."
  @spec fetch_restore_checkpoint(String.t(), GenServer.server()) ::
          {:ok, map()} | {:error, Error.t()}
  def fetch_restore_checkpoint(revision, server \\ __MODULE__),
    do: GenServer.call(server, {:fetch_restore_checkpoint, revision})

  @impl GenServer
  def init(%__MODULE__{} = state) do
    case recover(state) do
      :ok -> {:ok, state}
      {:error, %Error{} = error} -> {:stop, {:config_recovery_failed, error}}
      _other -> {:stop, {:config_recovery_failed, invalid_error()}}
    end
  end
  ...
  def handle_call({:fetch, version, digest}, _from, state) do
    reply =
      with {:ok, version} <- version(version),
           {:ok, digest} <- Digest.validate(digest) do
        read_manifest_version(%{version: version, digest: digest}, state)
      else
        _invalid -> invalid()
      end
  ...
  def handle_call({:fetch_revision, revision}, _from, state) do
    reply =
      with {:ok, revision} <- Digest.validate(revision),
           {:ok, pointer} <- revision_pointer(revision, state),
  ...
  def handle_call({:put_restore_checkpoint, version, digest, checkpoint}, _from, state) do
    reply = put_restore_checkpoint_document(version, digest, checkpoint, state)
    {:reply, reply, state}
  end

  def handle_call({:fetch_restore_checkpoint, revision}, _from, state) do
    reply =
      with {:ok, revision} <- Digest.validate(revision),
           {:ok, pointer} <- checkpoint_pointer(revision, state),
           {:ok, checkpoint} <- read_restore_checkpoint(pointer, state) do
        {:ok, checkpoint}
      else
        {:error, %Error{}} = error -> error
        _invalid -> invalid()
  ...
  defp recover(state) do
    with :ok <- ensure_storage_boundary(state),
         {:ok, entries} <- version_entries(state),
         true <- length(entries) <= state.max_versions,
         {:ok, checkpoints} <- checkpoint_entries(state),
         true <- length(checkpoints) <= state.max_versions,
         :ok <- validate_restore_checkpoints(checkpoints, state) do
      case read_final(manifest_path(state), state) do
        {:ok, document} ->
          with {:ok, manifest} <- decode_manifest(document, state),
               :ok <- validate_staged_versions(manifest, state) do
            :ok
          end
  ...
  defp validate_delivery(%Envelope{} = envelope, state) do
    with {:ok, _envelope} <- Operation.validate_envelope(envelope, :config),
         :ok <- exact_netman(envelope, state),
  ...
  defp put_restore_checkpoint_document(version, digest, checkpoint, state) do
    pointer = %{version: version, digest: digest}

    with {:ok, version} <- version(version),
         {:ok, digest} <- Digest.validate(digest),
         {:ok, candidate} <-
           read_manifest_version(%{pointer | version: version, digest: digest}, state),
         {:ok, restore_operation, restore_payload} <- restore_checkpoint(checkpoint),
         :ok <- compatible_restore(candidate["operation"], restore_operation),
         document = %{
           "schema_version" => 1,
           "target_type" => "netman",
           "target_id" => state.netman_id,
         ...
         {:ok, _path} <-
           create_checkpoint(checkpoint_path(state, version, digest), document, state),
         {:ok, ^document} <- read_restore_checkpoint(%{version: version, digest: digest}, state) do
      :ok
    else
      {:error, %Error{}} = error -> error
      _invalid -> invalid()
  ...
  defp restore_checkpoint(%{operation: operation, payload: payload} = checkpoint) do
    with true <- Enum.sort(Map.keys(checkpoint)) == [:operation, :payload],
         {:ok, operation} <- config_operation(operation, payload) do
  ...
  defp restore_checkpoint(_checkpoint), do: invalid()

  defp compatible_restore("netman.profiles.replace", "netman.profiles.replace"), do: :ok

  defp compatible_restore(
         candidate,
         "netman.resolved.config.rollback"
       )
       when candidate in [
  ...
  defp compatible_restore(_candidate, _restore), do: invalid()

  defp read_restore_checkpoint(pointer, state) do
    path = checkpoint_path(state, pointer.version, pointer.digest)

    with {:ok, document} <- read_version(path, state),
         {:ok, document} <- validate_restore_checkpoint(document, pointer, path, state) do
      {:ok, document}
    end
  end

  defp validate_restore_checkpoint(document, pointer, path, state) when is_map(document) do
    with true <- Enum.sort(Map.keys(document)) == @restore_checkpoint_keys,
         1 <- document["schema_version"],
         "netman" <- document["target_type"],
         true <- document["target_id"] == state.netman_id,
         true <- document["version"] == pointer.version,
         true <- document["candidate_digest"] == pointer.digest,
         {:ok, candidate} <- read_manifest_version(pointer, state),
         true <- document["candidate_operation"] == candidate["operation"],
         {:ok, restore_operation} <-
           config_operation(document["restore_operation"], document["restore_payload"]),
         :ok <- compatible_restore(candidate["operation"], restore_operation),
         true <- path == checkpoint_path(state, pointer.version, pointer.digest) do
      {:ok, document}
    else
      {:error, %Error{}} = error -> error
      _invalid -> invalid()
  ...
  defp validate_restore_checkpoint(_document, _pointer, _path, _state), do: invalid()

  defp validate_restore_checkpoints(entries, state) do
    Enum.reduce_while(entries, :ok, fn entry, :ok ->
      with {:ok, pointer} <- entry_to_pointer(entry),
           {:ok, _checkpoint} <- read_restore_checkpoint(pointer, state) do
        {:cont, :ok}
      else
  ...
  defp entry_to_pointer(entry) do
    with [version, digest_file] <- String.split(entry, "-", parts: 2),
         {version, ""} when version > 0 <- Integer.parse(version),
         digest <- String.trim_trailing(digest_file, ".json"),
         true <- digest_file == digest <> ".json",
         {:ok, digest} <- Digest.validate(digest) do
  ...
  defp decode_manifest(manifest, state) when is_map(manifest) do
    netman_id = state.netman_id

    with true <- Enum.sort(Map.keys(manifest)) == @manifest_keys,
         1 <- manifest["schema_version"],
         "netman" <- manifest["target_type"],
         ^netman_id <- manifest["target_id"],
  ...
  defp state_from_options(opts) when is_list(opts) do
    allowed = [:name, :data_dir, :netman_id, :max_bytes, :max_versions, :storage_opts]

    with true <- Keyword.keyword?(opts),
         true <- Enum.all?(Keyword.keys(opts), &(&1 in allowed)),
         {:ok, data_dir} <- absolute_data_dir(Keyword.get(opts, :data_dir)),
         {:ok, netman_id} <- nonempty_id(Keyword.get(opts, :netman_id)),
         {:ok, max_versions} <-
           max_versions(Keyword.get(opts, :max_versions, @default_max_versions)),
  ...
  defp max_versions(value)
       when is_integer(value) and value > 0 and value <= @maximum_max_versions,
       do: {:ok, value}

  defp max_versions(_value), do: :error
  ...
  defp exact_netman(%Envelope{target_type: :netman, target_id: target_id}, %{netman_id: target_id}),
       do: :ok

  defp exact_netman(_envelope, _state), do: invalid()

  defp config_operation(operation, payload) do
    with true <- operation in @config_operations,
         {:ok, %Operation{target_type: :netman, kind: :config} = concrete} <-
  ...
  defp create_checkpoint(path, document, state) do
    with :ok <- checked_final_path(path, state),
         {:ok, stored_path} <- Storage.create(path, document, state.storage_opts),
         :ok <- checked_final_path(path, state) do
      {:ok, stored_path}
  ...
  defp ensure_version_capacity(path, state) do
    case File.lstat(path) do
      {:ok, %File.Stat{type: :regular}} ->
        :ok

      {:error, :enoent} ->
        with {:ok, entries} <- version_entries(state),
             true <- length(entries) < state.max_versions do
          :ok
        else
          false -> version_limit()
          {:error, %Error{}} = error -> error
  ...
  defp version_entries(state) do
    with {:ok, entries} <- File.ls(versions_directory(state)),
         :ok <- validate_version_entries(entries, state) do
      {:ok, entries}
    else
      {:error, %Error{}} = error -> error
      _other -> invalid()
  ...
  defp checkpoint_entries(state) do
    with {:ok, entries} <- File.ls(checkpoints_directory(state)),
         :ok <- validate_checkpoint_entries(entries, state) do
      {:ok, entries}
    else
      {:error, %Error{}} = error -> error
      _other -> invalid()
  ...
  defp revision_pointer(revision, state) do
    with {:ok, entries} <- version_entries(state) do
      entries
      |> Enum.flat_map(&entry_pointer(&1, revision))
  ...
  defp checkpoint_pointer(revision, state) do
    with {:ok, entries} <- checkpoint_entries(state) do
      entries
      |> Enum.flat_map(&entry_pointer(&1, revision))
      |> Enum.max_by(& &1.version, fn -> nil end)
      |> case do
  ...
  defp entry_pointer(entry, revision) do
    suffix = "-#{revision}.json"

    if String.ends_with?(entry, suffix) do
      case entry |> String.trim_trailing(suffix) |> Integer.parse() do
        {version, ""} when version > 0 -> [%{version: version, digest: revision}]
        _invalid -> []
      end
    else
      []
  ...
  defp validate_version_entries(entries, state) when is_list(entries) do
    if Enum.all?(entries, &valid_version_entry?(&1, state)), do: :ok, else: invalid()
  end

  defp validate_version_entries(_entries, _state), do: invalid()

  defp validate_checkpoint_entries(entries, state) when is_list(entries) do
    if Enum.all?(entries, &valid_checkpoint_entry?(&1, state)), do: :ok, else: invalid()
  end

  defp validate_checkpoint_entries(_entries, _state), do: invalid()

  defp valid_version_entry?(entry, state) when is_binary(entry) do
    Regex.match?(~r/\A[1-9][0-9]*-[0-9a-f]{64}\.json\z/, entry) and
      match?(
        {:ok, %File.Stat{type: :regular}},
        File.lstat(Path.join(versions_directory(state), entry))
      )
  end

  defp valid_version_entry?(_entry, _state), do: false

  defp valid_checkpoint_entry?(entry, state) when is_binary(entry) do
    Regex.match?(~r/\A[1-9][0-9]*-[0-9a-f]{64}\.json\z/, entry) and
      match?(
        {:ok, %File.Stat{type: :regular}},
        File.lstat(Path.join(checkpoints_directory(state), entry))
      )
  end

  defp valid_checkpoint_entry?(_entry, _state), do: false
  ...
  defp ensure_storage_boundary(state) do
    with :ok <- ensure_owned_directory(state.data_dir),
         :ok <- ensure_owned_directory(netman_directory(state)),
         :ok <- ensure_owned_directory(versions_directory(state)),
         :ok <- ensure_owned_directory(checkpoints_directory(state)) do
      :ok
    else
      {:error, %Error{}} = error -> error
      _other -> invalid()
  ...
  defp netman_directory(state), do: Path.join(state.data_dir, "netman")

  defp versions_directory(state), do: Path.join(netman_directory(state), "versions")

  defp checkpoints_directory(state), do: Path.join(netman_directory(state), "restore-checkpoints")

  defp manifest_path(state), do: Path.join(netman_directory(state), "manifest.json")

  defp version_path(state, version, digest),
    do: Path.join(versions_directory(state), "#{version}-#{digest}.json")

  defp checkpoint_path(state, version, digest),
    do: Path.join(checkpoints_directory(state), "#{version}-#{digest}.json")

  defp invalid,
    do: {:error, %Error{code: :invalid, message: "invalid config document", details: %{}}}

  defp invalid_error,
    do: %Error{code: :invalid, message: "invalid config document", details: %{}}

  defp conflict,
    do: {:error, %Error{code: :conflict, message: "config version conflicts", details: %{}}}

  defp version_limit,
    do: {:error, %Error{code: :conflict, message: "config version limit reached", details: %{}}}
