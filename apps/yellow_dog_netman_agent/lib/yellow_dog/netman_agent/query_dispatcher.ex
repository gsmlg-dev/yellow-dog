defmodule YellowDog.NetmanAgent.QueryDispatcher do
  @moduledoc """
  Stateless query-only dispatch boundary for local Netman reads.
  """
  ...
  @allowed_options [:netman_id, :capabilities, :runtime_adapter]
  @error_messages %{
  ...
  defp validate_query(envelope, config) do
    with {:ok, %Envelope{} = envelope} <- Operation.validate_envelope(envelope, :query),
         ...
         {:ok, %Operation{target_type: :netman, kind: :query} = operation} <-
