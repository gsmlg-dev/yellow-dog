defmodule YellowDog.Netboot.Asset.ManagedAsset do
  ...
  def tombstone_filename(%__MODULE__{} = asset) do
    digest =
      :crypto.hash(:sha256, asset.asset_id)
      |> Base.encode16(case: :lower)

    filename = ".yellowdog-delete-#{digest}.tombstone"

    case Path.dirname(asset.filename) do
      "." -> filename
  ...
  defp validate_tombstone(_filename, nil, :active), do: :ok

  defp validate_tombstone(filename, tombstone_filename, :tombstoned) do
    with :ok <- validate_filename(tombstone_filename, :invalid_tombstone_filename),
         true <- Path.dirname(filename) == Path.dirname(tombstone_filename),
         true <- filename != tombstone_filename do
      :ok
    else
  ...
  defp validate_tombstone(_filename, _tombstone_filename, _lifecycle),
