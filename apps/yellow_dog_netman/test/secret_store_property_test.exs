defmodule YellowDog.Netman.SecretStorePropertyTest do
  ...
  property "put with list key does not raise" do
    check all(
            key <-
              StreamData.list_of(StreamData.string(:alphanumeric, max_length: 4), max_length: 5),
  ...
  property "put followed by get always returns the stored value" do
    check all(
            ...
            val <- StreamData.string(:printable, min_length: 0, max_length: 50)
          ) do
  ...
  property "SecretStore put with binary key always returns ok (r63)" do
    check all(key <- StreamData.string(:alphanumeric, min_length: 1, max_length: 30)) do
