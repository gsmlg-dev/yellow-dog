defmodule YellowDogCore.Server do
  @moduledoc """
  ...
  @spec start_link(YellowDogCore.ServerConfig.t()) :: Supervisor.on_start()
  def start_link(%YellowDogCore.ServerConfig{} = config) do
    Supervisor.start_link(__MODULE__, config, config.supervisor_options)
  end

  @impl Supervisor
  @spec init(YellowDogCore.ServerConfig.t()) ::
          {:ok,
           {Supervisor.sup_flags(),
            [Supervisor.child_spec() | (old_erlang_child_spec :: :supervisor.child_spec())]}}
  def init(config) do
    udp_config = YellowDogCore.ServerConfig.udp_config(config)

    children = [
      {Abyss, udp_config}
      |> Supervisor.child_spec(id: :udp_server),
      {YellowDogDns.ViewManager, []}
