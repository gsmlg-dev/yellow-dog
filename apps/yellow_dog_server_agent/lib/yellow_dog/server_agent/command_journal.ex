defmodule YellowDog.ServerAgent.CommandJournal do
  @moduledoc """
  Durable local command reservation and terminal replay for one Server.
  """
  ...
  def handle_call(:wire_projection, _from, state) do
    message = %Journal{
      target_type: :server,
      target_id: state.config.server_id,
  ...
  defp validate_envelope(envelope, config) do
    with {:ok, %Envelope{} = envelope} <- Operation.validate_envelope(envelope, :command),
         ...
         {:ok, %Operation{target_type: :server, kind: :command} = operation} <-
           Operation.lookup(envelope.operation),
         true <- MapSet.member?(config.capabilities, operation.capability) do
      {:ok, envelope}
    else
      _invalid -> invalid("invalid Server command envelope")
    end
  end
    ...
    if Keyword.keyword?(opts) do
      with {:ok, data_dir} <- absolute_data_dir(Keyword.get(opts, :data_dir)),
        ...
        {:ok,
         %{
           data_dir: data_dir,
           directory: Path.join([data_dir, "server", "journals"]),
           server_id: server_id,
  ...
  defp decode_record(document, expected_request_id, path, server_id) when is_map(document) do
    with true <- exact_keys?(document, @document_keys),
         ...
         true <- envelope.target_type == :server,
         true <- envelope.target_id == server_id,
         true <- envelope.request_id == expected_request_id,
         {:ok, %Operation{target_type: :server, kind: :command}} <-
           Operation.lookup(envelope.operation),
  ...
  defp decode_record(_document, _expected_request_id, _path, _server_id),
    do: {:error, :corrupt}
  ...
  defp decode_outcome(:succeeded, envelope, result, nil, resolved_at, updated_at)
    ...
    with true <- resolved_at == updated_at,
         {:ok, result} <-
           Operation.validate_result(envelope.operation, :server, :command, result) do
      {:ok, result, nil}
    end
  end
...
defmodule YellowDog.ServerAgent.CommandJournal.DirectoryScanner do
  @moduledoc false
