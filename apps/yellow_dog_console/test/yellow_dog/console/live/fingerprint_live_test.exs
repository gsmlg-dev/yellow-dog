defmodule YellowDog.Console.FingerprintLiveTest do
  ...
  describe "Device Inventory page" do
    ...
    test "has export CSV button", %{conn: conn} do
      {:ok, view, _html} = live(conn, "/system/fingerprint/devices")
  ...
  describe "Fingerprints page" do
    ...
    test "has export CSV button", %{conn: conn} do
      {:ok, view, _html} = live(conn, "/system/fingerprint/fingerprints")
  ...
  describe "Device Detail page" do
    ...
    test "shows device not found for unknown MAC", %{conn: conn} do
      {:ok, _view, html} = live(conn, "/system/fingerprint/devices/ff:ff:ff:ff:ff:ff")

      assert html =~ "Device not found"
    end

    test "has back link to inventory", %{conn: conn} do
      {:ok, view, _html} = live(conn, "/system/fingerprint/devices/00:11:22:33:44:55")

      assert has_element?(view, "a[href='/system/fingerprint/devices']")
    end
  end
end
